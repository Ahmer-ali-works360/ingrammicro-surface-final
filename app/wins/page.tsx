...
export default function Page() {
    ...
    const validateField = (name: string, value: any) => {
        ...
        switch (name) {
            ...
            case "resellerName":
                ...
                break;
            case "ingramOrderNumber":
                if (!value.trim()) error = "ingram order number is required";
                break;
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (validateForm()) {
            try {
                ...
                const orderHash = selectedOrder.orderHash || formData.ingramOrderNumber;
    ...
    const sendWinEmail = async (oData: any) => {
        ...
        try {
            ...
            // Use internal order_no for Win Reported Order# and orderHash for ingram Order#
            const template = emailTemplates.reportWinEmail({
                orderNumber: oData.order_no || "N/A", // Internal order number for "Win Reported Order#"
                ingramOrderNumber: oData.orderHash || "N/A", // ingram order number for "ingram Order#"
            ...
            <div className="max-w-6xl mx-auto">
                ...
                <form onSubmit={handleSubmit} className="bg-white shadow-xl rounded-xl p-5 sm:p-6 md:p-8">
                            ...
                            <div className="space-y-4">
                                ...
                                {loadingProducts ? (
                                    <div className="flex justify-center items-center p-4 border border-gray-200 rounded-lg">
                                        <div className="animate-spin rounded-full h-5 w-5 border-b-2 border-[#1D76BC]"></div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                        ...
                        <div>
                            <label className="block text-sm font-semibold text-gray-700 mb-2">
                                Ingram Order# <span className="text-red-600">*</span>
                            </label>
                            <input
                                type="text"
                                name="ingramOrderNumber"
                                value={formData.ingramOrderNumber}
                                onChange={handleChange}
                                className={`w-full rounded-lg border px-4 py-3 text-gray-900 focus:outline-none transition text-sm ${errors.ingramOrderNumber
                                    ? "border-red-500 focus:ring-2 focus:ring-red-300"
                                    : "border-gray-300 focus:border-[#1D76BC] focus:ring-2 focus:ring-[#1D76BC]/30"
                                    }`}
                            
                            />
                            {errors.ingramOrderNumber && (
                                <p className="mt-1 text-sm text-red-600">{errors.ingramOrderNumber}</p>
                            )}
                        </div>
                    </div>
                    ...
                    <div className="mb-8">
                        <label className="block text-sm font-semibold text-gray-700 mb-2">
                            How did Ingram micro Surface help you close this deal? <span className="text-red-600">*</span>
                        </label>
                        <textarea
                            name="howHelped"
