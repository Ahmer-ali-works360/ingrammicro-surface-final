...
export async function POST(request: Request) {
    try {
        ...
        // Supabase Admin API mein direct email filter support nahi — pagination zaroori hai
        // Lekin ab Promise-based parallel fetch karte hain pehle page se
        let page = 1
        const perPage = 200

        outer: while (true) {
            const { data: usersPage, error: listErr } = await admin.auth.admin.listUsers({ page, perPage })
            console.log('wp-upgrade:listUsers', { page, count: usersPage?.users?.length, error: listErr?.message })
        ...
        // ✅ Teeno operations parallel chalao — waterfall khatam
        const [
            { error: updErr },
            { error: userUpdErr },
            { error: delErr }
        ] = await Promise.all([
            admin.auth.admin.updateUserById(targetUserId, { password }),
            admin.from('users').update({ userId: targetUserId, password: null }).eq('email', email),
            admin.from('wp_pass').delete().eq('email', email)
        ])

        console.log('wp-upgrade:parallelOps', {
            email,
            userId: targetUserId,
            updErr: updErr?.message,
            userUpdErr: userUpdErr?.message,
            delErr: delErr?.message,
        })

        // ✅ wp_pass delete fail — hard fail nahi, kaam ho chuka hai
        if (delErr) {
        ...
        console.log('wp-upgrade:catch', e)
